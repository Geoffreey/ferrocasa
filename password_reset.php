...
// Solicitud de publicación: restablecer contraseña mediante código de restablecimiento de contraseña
if ($request->server['REQUEST_METHOD'] == 'POST' && $request->get['action_type'] == "RESET")
{
  try {
    ...
    // Hacer coincidir la contraseña y confirmar la contraseña
...
// Compruebe si el código de restablecimiento de contraseña existe o no
$time = time()-(24*60)*60;
$statement = db()->prepare("SELECT * FROM `users` WHERE `pass_reset_code` = ? AND `reset_code_time` > NOW() - $time");
$statement->execute(array($reset_code));
$user = $statement->fetch(PDO::FETCH_ASSOC);
...
<div class="hidden"><?php include('assets/itsolution24/img/iconmin/membership/membership.svg');?></div>
  ...
  <section class="login-box">
    ...
    <div class="copyright text-center">
      <p>&copy; <a href="http://geoffdeep.pw">geoffdeep.pw</a>, v<?php echo settings('version'); ?></p>
    </div>
  </section>
