window.angularApp.controller("QuotationController", [
  ...
  function (
        ...
        columnDefs: [
          ...
          { className: "text-center", targets: [0, 2, 3, 5, 6] },
          {
            ...
            createdCell: function (td, cellData, rowData, row, col) {
              $(td).attr(
                "data-title",
                $("#quotation-quotation-list thead tr th:eq(0)").html()
              );
            },
          },
          {
            ...
            createdCell: function (td, cellData, rowData, row, col) {
              $(td).attr(
                "data-title",
                $("#quotation-quotation-list thead tr th:eq(1)").html()
              );
            },
          },
          {
            ...
            createdCell: function (td, cellData, rowData, row, col) {
              $(td).attr(
                "data-title",
                $("#quotation-quotation-list thead tr th:eq(2)").html()
              );
            },
          },
          {
            ...
            createdCell: function (td, cellData, rowData, row, col) {
              $(td).attr(
                "data-title",
                $("#quotation-quotation-list thead tr th:eq(3)").html()
              );
            },
          },
          {
            ...
            createdCell: function (td, cellData, rowData, row, col) {
              $(td).attr(
                "data-title",
                $("#quotation-quotation-list thead tr th:eq(4)").html()
              );
            },
          },
          {
            ...
            createdCell: function (td, cellData, rowData, row, col) {
              $(td).attr(
                "data-title",
                $("#quotation-quotation-list thead tr th:eq(5)").html()
              );
            },
          },
          {
            ...
            createdCell: function (td, cellData, rowData, row, col) {
              $(td).attr(
                "data-title",
                $("#quotation-quotation-list thead tr th:eq(6)").html()
              );
            },
          },
        ],
    ...
    $(document).delegate("#view_customer", "click", function (e) {
      ...
      window.open(
        window.baseUrl + "/admin/customer_profile.php?customer_id=" + customerID
      );
    });
    ...
    $scope._calculateTotalPayable = function () {
      ...
      if ($scope.orderTax < 1 || $scope.orderTax > 100) {
        ...
        $scope.taxInput =
          (parseFloat($scope.orderTax) / 100) * parseFloat(total);
      }
      payableAmount =
        parseFloat(total) +
    ...
    $scope.addProduct = function (data) {
      if (data.itemTaxMethod == "exclusive") {
        sellPrice =
          parseFloat(data.itemSellPrice) * parseFloat(data.itemQuantity) +
      ...
      html +=
        ...
        data.itemId +
        '">' +
        window.formatDecimal(data.available, 2) +
        "</span>";
      html += "</td>";
      ...
      html +=
        '<input id="unit-price-' +
        data.itemId +
        '" class="form-control input-sm text-center unit-price" type="text" name="products[' +
        data.itemId +
        '][unit_price]" value="' +
        data.itemSellPrice +
        '" data-id="' +
      ...
      html +=
        ...
        data.itemId +
        '">' +
        window.formatDecimal(sellPrice, 2) +
        "</span>";
      html += "</td>";
      ...
      if ($("#" + data.itemId).length) {
        ...
        subTotal.text(
          window.formatDecimal(
            parseFloat(subTotal.text()) + parseFloat(sellPrice),
            2
          )
        );
      } else {
    ...
    if (window.getParameterByName("reference_no")) {
      ...
      $http({
        ...
        function (data) {
          ...
          $scope.payableAmount = quotation.payable_amount;
          $scope.orderTax = quotation.order_tax;
          $scope.shippingAmount = window.formatDecimal(
            quotation.shipping_amount,
            2
          );
          $scope.othersCharge = window.formatDecimal(
            quotation.others_charge,
            2
          );
          $scope.discountAmount = window.formatDecimal(
    ...
    if (window.sendReportEmail) {
      $(".dt-buttons").append(
        '<button id="email-btn" class="btn btn-default buttons-email" tabindex="0" aria-controls="quotation-quotation-list" type="button" title="Email"><span><i class="fa fa-envelope"></i></span></button>'
      );
    }

    // Send quotation list through email
    $("#email-btn").on("click", function (e) {
      e.stopPropagation();
      e.preventDefault();
      dt.find(
        "thead th:nth-child(7), tbody td:nth-child(7), tfoot th:nth-child(7)"
      ).addClass("hide-in-mail");
      var thehtml = dt.html();
      EmailModal({
        template: "default",
        subject: "Quotation Listing",
        title: "Quotation Listing",
        html: thehtml,
      });
    });
