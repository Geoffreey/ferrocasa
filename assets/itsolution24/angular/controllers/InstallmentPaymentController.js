window.angularApp.controller("InstallmentPaymentController", [
  ...
  function (
    ...
    $("#payment-payment-list").dataTable({
      ...
      ajax:
        API_URL +
        "/_inc/installment_payment.php?from=" +
        $from +
        "&to=" +
        $to +
        "&type=" +
      ...
      columnDefs: [
        ...
        { visible: false, targets: hideColumsArray },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#payment-payment-list thead tr th:eq(0)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#payment-payment-list thead tr th:eq(1)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#payment-payment-list thead tr th:eq(2)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#payment-payment-list thead tr th:eq(3)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#payment-payment-list thead tr th:eq(4)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#payment-payment-list thead tr th:eq(5)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#payment-payment-list thead tr th:eq(6)").html()
            );
          },
        },
      ],
      ...
      buttons: [
        ...
        },
        {
          ...
          titleAttr: "Copy",
          title:
            window.store.name + " > Installment Listing-" + from + " to " + to,
          exportOptions: {
            columns: [0, 1, 2, 3, 4, 5],
          },
        },
    ...
    if (window.sendReportEmail) {
      $(".dt-buttons").append(
        '<button id="email-btn" class="btn btn-default buttons-email" tabindex="0" aria-controls="payment-payment-list" type="button" title="Email"><span><i class="fa fa-envelope"></i></span></button>'
      );
    }
    ...
    $("#email-btn").on("click", function (e) {
      ...
      EmailModal({
        template: "default",
        subject: "Installment Payment Listing",
        title: "Installment Payment Listing",
        html: thehtml,
      });
    });
  },
