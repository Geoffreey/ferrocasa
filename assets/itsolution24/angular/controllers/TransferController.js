  ...
  function (
    ...
    dt.dataTable({
      ...
      columnDefs: [
        ...
        { className: "text-center", targets: [0, 1, 6, 7] },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#transfer-transfer-list thead tr th:eq(1)").html()
            );
          },
        },
        {
        ...
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#transfer-transfer-list thead tr th:eq(2)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#transfer-transfer-list thead tr th:eq(3)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#transfer-transfer-list thead tr th:eq(4)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#transfer-transfer-list thead tr th:eq(5)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#transfer-transfer-list thead tr th:eq(6)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#transfer-transfer-list thead tr th:eq(7)").html()
            );
          },
        },
      ],
      ...
      footerCallback: function (row, data, start, end, display) {
        ...
        $(api.column(4).footer()).html(formatDecimal(pageTotal, 2));
        ...
        // Actualizar pie de página
        $(api.column(5).footer()).html(formatDecimal(pageTotal, 2));
      },
      ...
      buttons: [
        {
          ...
          customize: function (win) {
            $(win.document.body)
              .css("font-size", "10pt")
              .append(
                "<div><b><i>Developed & Maintained by: geoffdeep.pw</i></b></div>"
              )
              .prepend(
                '<div class="dt-print-heading"><img class="logo" src="' +
                  baseUrl +
                  '/assets/itsolution24/img/logo-favicons/1_logo.png"/><h2 class="title">' +
                  window.store.name +
                  "</h2><p><b><i>From</i> " +
                  window.from +
                  " <i>To</i> " +
        ...
        },
        {
          ...
          titleAttr: "Copy",
          title:
            window.store.name + " > Transfer Listing-" + from + " to " + to,
          exportOptions: {
            columns: [0, 1, 2, 3, 4, 5],
          },
        },
    ...
    $scope.showStockList = function (toStoreID) {
      ...
      $http({
        url:
          window.baseUrl +
          "/_inc/ajax.php?type=STOCKITEMS&store_id=" +
          toStoreID +
          "&searchTerm=" +
          $scope.searchTerm,
        method: "GET",
    ...
    $scope.addItemToTransferList = function (id, qty, index) {
      ...
      $http({
        url:
          API_URL +
          "/_inc/ajax.php?type=STOCKITEM&id=" +
          id +
          "&quantity=" +
          qty,
        method: "GET",
    ...
    if (window.sendReportEmail) {
      $(".dt-buttons").append(
        '<button id="email-btn" class="btn btn-default buttons-pdf buttons-email" tabindex="0" aria-controls="transfer-transfer-list" type="button" title="Email"><span><i class="fa fa-envelope"></i></span></button>'
      );
    }
    ...
    $("#email-btn").on("click", function (e) {
      ...
      e.preventDefault();
      dt.find(
        "thead th:nth-child(5), thead th:nth-child(6), thead th:nth-child(7), tbody td:nth-child(5), tbody td:nth-child(6), tbody td:nth-child(7), tfoot th:nth-child(5), tfoot th:nth-child(6), tfoot th:nth-child(7)"
      ).addClass("hide-in-mail");
      var thehtml = dt.html();
      EmailModal({
        template: "default",
        subject: "Send Transter List",
        title: "Send Transter List",
        html: thehtml,
      });
    });
  },
