  ...
  function (
    ...
    $("#invoice-invoice-list").dataTable({
      ...
      columnDefs: [
        ...
        { visible: false, targets: hideColumsArray },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(0)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(1)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(2)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(3)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(4)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(5)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(6)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(7)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(8)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(9)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(10)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(11)").html()
            );
          },
        },
        {
          ...
          createdCell: function (td, cellData, rowData, row, col) {
            $(td).attr(
              "data-title",
              $("#invoice-invoice-list thead tr th:eq(12)").html()
            );
          },
        },
      ],
      ...
      footerCallback: function (row, data, start, end, display) {
        ...
        $(api.column(4).footer()).html(window.formatDecimal(pageTotal, 2));
        ...
        // Actualizar pie de página
        $(api.column(5).footer()).html(window.formatDecimal(pageTotal, 2));
        ...
        // Actualizar pie de página
        $(api.column(6).footer()).html(window.formatDecimal(pageTotal, 2));
      },
      ...
      buttons: [
        ...
        },
        {
          ...
          titleAttr: "Copy",
          title:
            window.store.name + " > Purchase Listing-" + from + " to " + to,
          exportOptions: {
            columns: [0, 1, 2, 3, 4, 5, 6],
          },
        },
    ...
    // Pago de Table Selection Modal [for Dinein order type]
    ...
    $(document).delegate("#return_item", "click", function (e) {
      ...
      $http({
        url:
          window.baseUrl +
          "/_inc/purchase_payment.php?action_type=ORDERDETAILS&invoice_id=" +
          d.invoice_id,
        method: "GET",
      }).then(
    ...
    $scope._calculate = function (id) {
      ...
      realItemTaxAmount = parseFloat(
        (itemTaxrate.val() / 100) * parseFloat(unitPrice.val())
      );
      itemTaxAmount.val(parseFloat(quantity.val()) * realItemTaxAmount);
      taxAmount.val(
        parseFloat(parseFloat(quantity.val()) * realItemTaxAmount).toFixed(2)
      );
      ...
      if (itemTaxMethod.val() == "exclusive") {
        subTotal.text(
          parseFloat(
            parseFloat(quantity.val()) * parseFloat(unitPrice.val()) +
              parseFloat(taxAmount.val())
          ).toFixed(2)
        );
      } else {
        subTotal.text(
    ...
    $scope._calculateTotalPayable = function () {
      ...
      if ($scope.orderTax < 1 || $scope.orderTax > 100) {
        ...
        $scope.taxInput =
          (parseFloat($scope.orderTax) / 100) * parseFloat(total - totalTax);
      }
      ...
      payableAmount =
        parseFloat(total) +
        parseFloat($scope.taxInput) +
        parseFloat($scope.shippingAmount) +
        parseFloat($scope.othersCharge) -
        parseFloat($scope.discountAmount);
    ...
    $scope.addPaidAmount = function () {
      $scope.$applyAsync(function () {
        $scope.dueAmount =
          parseFloat($scope.payableAmount) > parseFloat($scope.paidAmount)
            ? parseFloat($scope.payableAmount) - parseFloat($scope.paidAmount)
            : 0;
        $scope.changeAmount =
          parseFloat($scope.payableAmount) < parseFloat($scope.paidAmount)
            ? parseFloat($scope.paidAmount) - parseFloat($scope.payableAmount)
            : 0;
      });
    };
    ...
    $scope.addProduct = function (data) {
      if (data.itemTaxMethod == "exclusive") {
        purchasePrice =
          parseFloat(data.itemPurchasePrice) * parseFloat(data.itemQuantity) +
          parseFloat(data.itemTaxAmount);
      } else {
        purchasePrice =
          parseFloat(data.itemPurchasePrice) * parseFloat(data.itemQuantity);
      }
      ...
      html +=
        '<input name="products[' +
        data.itemId +
        '][item_id]" type="hidden" class="item-id" value="' +
        data.itemId +
        '">';
      html +=
        '<input name="products[' +
        data.itemId +
        '][item_name]" type="hidden" class="item-name" value="' +
        data.itemName +
        '">';
      html +=
        '<input name="products[' +
        data.itemId +
        '][category_id]" type="hidden" class="categoryid" value="' +
        data.categoryId +
        '">';
      html +=
      ...
      html +=
        '<span class="text-center available" id="available-' +
        data.itemId +
        '">' +
        window.formatDecimal(data.itemAvailable, 2) +
        "</span>";
      html += "</td>";
      html += '<td style="padding:2px;" data-title="Quantity">';
      html +=
        '<input class="form-control input-sm text-center quantity" name="products[' +
        data.itemId +
        ...
        '" id="quantity-' +
        data.itemId +
        '" onclick="this.select();" onkeypress="return IsNumeric(event);" ondrop="return false;" onpaste="return false;" onKeyUp="if(this.value<0){this.value=\'1\';}">';
      html += "</td>";
      html +=
      ...
      html +=
        ...
        data.itemId +
        '" class="form-control input-sm text-center purchase-price" type="text" name="products[' +
        data.itemId +
        '][purchase_price]" value="' +
        data.itemPurchasePrice +
        '" data-id="' +
        data.itemId +
        '" data-item="' +
        data.itemId +
      ...
      html +=
        '<input id="sell-price-' +
        data.itemId +
        '" class="form-control input-sm text-center sell-price" type="text" name="products[' +
        data.itemId +
        '][sell_price]" value="' +
        data.itemSellPrice +
        '" data-id="' +
      ...
      html +=
        ...
        '" name="products[' +
        data.itemId +
        '][tax_amount]" type="hidden" value="' +
        data.itemTaxAmount +
        '">';
      html +=
        '<span id="tax-amount-view-' +
        data.itemId +
        '" class="tax tax-amount-view">' +
        window.formatDecimal(data.itemTaxAmount, 2) +
        "</span>";
      html += "</td>";
      ...
      html +=
        ...
        data.itemId +
        '">' +
        window.formatDecimal(purchasePrice, 2) +
        "</span>";
      html += "</td>";
      html += '<td class="text-center">';
      html +=
        '<i class="fa fa-close text-red pointer remove" data-id="' +
        data.itemId +
        '" title="Remove"></i>';
      html += "</td>";
      html += "</tr>";
      ...
      if ($("#" + data.itemId).length) {
        ...
        subTotal.text(
          window.formatDecimal(
            parseFloat(subTotal.text()) + parseFloat(purchasePrice),
            2
          )
        );
      } else {
    ...
    $(document).delegate("#reset", "click", function (e) {
      ...
      $("#paid-amount").val(0);
      $("#image_thumb img").attr(
        "src",
        "../assets/itsolution24/img/noimage.jpg"
      );
      $("#image").val("");
    });
    ...
    if (window.sendReportEmail) {
      $(".dt-buttons").append(
        '<button id="email-btn" class="btn btn-default buttons-email" tabindex="0" aria-controls="purchase-purchase-list" type="button" title="Email"><span><i class="fa fa-envelope"></i></span></button>'
      );
    }
    ...
    $("#email-btn").on("click", function (e) {
      ...
      e.preventDefault();
      dt.find(
        "thead th:nth-child(9), thead th:nth-child(10), thead th:nth-child(11), thead th:nth-child(12), thead th:nth-child(13), tbody th:nth-child(9), tbody th:nth-child(10), tbody td:nth-child(11) tbody td:nth-child(12), tbody td:nth-child(13), tfoot td:nth-child(9), tfoot td:nth-child(10), tfoot td:nth-child(11), tfoot th:nth-child(12) tfoot th:nth-child(13)"
      ).addClass("hide-in-mail");
      var thehtml = dt.html();
      EmailModal({
        template: "default",
        subject: "Purchase Invoice Listing",
