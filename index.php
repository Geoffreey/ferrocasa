...
/*
| -----------------------------------------------------
| NOMBRE DEL PRODUCTO: Modern POS - Punto de venta con sistema de gestión de stock
| -----------------------------------------------------
| AUTOR geoffdeep.com
| -----------------------------------------------------
...
// Comprobar, si el usuario inicia sesión o no
if ($user->isLogged()) {
...
if ($request->server['REQUEST_METHOD'] == 'POST' && $request->get['action_type'] == "LOGIN")
{
  try {
    ...
    // Comprobar el total de intentos
...
// Envío de código de restablecimiento de contraseña por correo electrónico
if ($request->server['REQUEST_METHOD'] == 'POST' && $request->get['action_type'] == "SEND_PASSWORD_RESET_CODE")
{
  try {
    ...
    // Verifique, si la dirección de correo electrónico existe en la base de datos o no
    ...
    // Compruebe si el servidor SMTP está habilitado o no
    ...
    // Inicio de correo electrónicoo

    require_once('_inc/vendor/PHPMailer/PHPMailerAutoload.php');
    $mail = new PHPMailer;
    $mail->IsSMTP();    // Indicar a la clase que use SMTP
    $mail->SMTPAuth = true;     // Habilitar la autenticación SMTP
    $mail->SMTPSecure = $ssl_tls;     // Establece el prefijo en el servidor
    $mail->Host = $smtp_host;     // Establece GMAIL como servidor SMTP
    $mail->Port = $smtp_port;     // Establecer el puerto SMTP para el servidor GMAIL
    $mail->Username = $smtp_username;     // Nombre de usuario de GMAIL
    $mail->Password = $smtp_password;     // Contraseña de GMAIL
    ...
    // Actualizar el código de restablecimiento de contraseña de los usuarios
...
<div class="hidden"><?php include('assets/itsolution24/img/iconmin/membership/membership.svg');?></div>
  ...
  <?php if (!DEMO) : ?>
    <!--Inicio Olvidé mi contraseña modal-->
    ...
    <!-- Final Olvidé la contraseña Modal-->
  <?php endif; ?>
